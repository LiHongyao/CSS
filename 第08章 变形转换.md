...
[`transform`](https://developer.mozilla.org/zh-CN/docs/Web/CSS/transform) 用于对元素进行 **位移（translate）**、**缩放（scale）**、**旋转（rotate）** 和 **倾斜（skew）** 等变形处理，而不会影响其他元素的布局。
...
- `function`：变形类型，如 translate()、rotate()、scale() 等。
- `value`：具体的变形参数，如 50px、45deg。
